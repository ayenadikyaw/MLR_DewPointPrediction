...
def plot_y_test(regr, X_test, y_test, ask_user):
    ...
    if (range_start < datetime(2016,1,2,0,0,0) or range_end > datetime(2017,11,1,0,0,0) ):
...
def evaluation(X_train, y_train, X_test, y_test, poly_degree, interaction_only, print_coef, plot, ask_user, model_result, model_name, model_runtime, regr, alpha):
    ...
    print("R2_score: {0:.3f} \n".format(R2))
    NSE=1-(np.divide(np.sum(np.square(predict_test-column_or_1d(y_test))),(np.sum(np.square(predict_test-np.mean(column_or_1d(y_test)))) ))) 
    print("Nash Sutcliffe efficiency: {0:.3f} \n".format(NSE))
    # The mean squared error
...
def create_new_features(df):
    ...
    if ("WINDDIRECTION" in df.columns):
        ...
        df["cos_wind_dir"] = df["WINDDIRECTION"].apply(lambda x : math.cos( math.radians(x) ) )
...
# s_features = [] 
s_features = ['PRESSURE','MINTEMP','MAXTEMP','AVGTEMP','HUMIDITY', 'WINDDIRECTION','SPEED','RAINFALL']
...
#features_seasonal_only=['mins_year','mins_day']
features_seasonal_only=['cos_mins_year','cos_mins_day']
...
df = pd.read_csv('KYAUKPHYU.csv', usecols=fields, parse_dates=date)
#s1 = df[ df.STATION == 'WBAN:23244' ]
s2 = df[ df.STATION == 'KYAUKPHYU' ]
...
# for experiment in [0,1,2,3]:
    ...
    if   (experiment == 0):
        ...
        new_features = targets + ['MINTEMP']
    elif (experiment == 2):
        new_features = targets + ['PRESSURE']
    elif (experiment == 3):
        new_features = targets + ['HUMIDITY']
    elif (experiment==4):
        new_features= targets+['MINTEMP','PRESSURE','HUMIDITY']
    elif (experiment==5):
        new_features= targets+['mins_day','mins_year']
    elif (experiment==6):
        new_features= targets+['cos_mins_day','cos_mins_year']
    elif (experiment==7):
        new_features= targets+features_seasonal_only+['mins_day','mins_year']
    elif (experiment==8):
        new_features= targets+features_seasonal_only+['MINTEMP','mins_day','mins_year']
    else:
    ...
    with open("logs/log_" + log_timestr +".txt", "a") as logfile:
       # logfile.write("total_runtime = {} (seconds with open("logs/log_" + log_timestr +".txt", "a") as logfile:
        logfile.write("total_runtime={}\n".format(total_runtime))
        logfile.write("===================== \n")
